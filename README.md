...
scripts/add_noise.py
    └── writes ModifiedNoise/{video_id}/{noise_type}/{video_id}_{noise_type}_{severity}.mp4
    │
    ▼
...
`onnxruntime` itself is only imported if it's installed -- `add_noise.py` and `split_video.py`
work fine under the base `environment.yml` install with no GPU stack at all.
...
ModifiedNoise/
  {video_id}/
    illumination/*.mp4     e.g. {video_id}_illum_mild.mp4
    sensor_noise/*.mp4     e.g. {video_id}_sensor_mild.mp4
    motion_blur/*.mp4      e.g. {video_id}_motion_mild.mp4
```

This is exactly what `add_noise.py` writes. `video_id` and `noise_type` are read off the two path
components directly above each file, not off the filename, so the files themselves can be named
anything. There is no severity sub-folder in this layout -- `run_pipeline.py` currently writes the
`severity` column in `ps_features_ALL_NOISE_VIDEOS.csv` as a copy of `noise_type`, since there's
nothing on the path to read a real severity from. The true severity (`clean` / `mild` / `moderate`
/ `strong`) only exists in each file's name suffix; recover it from there if you need it for
analysis.
...
@software{vadavalli_ps_noise_robustness,
  ...
  url     = {https://github.com/shriyavad03/ps-noise-robustness},
  license = {MIT}
}
```
